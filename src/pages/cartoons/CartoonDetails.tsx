...
import CartoonDetailsRelatedList from "./CartoonDetailsRelatedList";
...
const CartoonDetails: FC = () => {
  const {
    cartoonId: cartoonImagesId,
    cartoonId: overviewDetailsId,
    cartoonId: detailsId,
    cartoonId: reviewId,
  } = useParams();
  ...
  } = useGetImagesQuery(cartoonImagesId || "");
  ...
  const {
    data: cartoonDetails,
    isFetching: isFetchingCartoonDetails,
    isError: isErrorCartoonDetails,
  } = useGetDetailsQuery(detailsId || "");
  let DATA = cartoonDetails?.d[0];

  const {
    data: relatedCartoons,
    isFetching: relatedCartoonsFetching,
    isError: relatedCartoonsError,
  } = useGetRelatedCartoonsQuery([]);
  ...
  if (
    isFetchingImages &&
    isFetching &&
    relatedCartoonsFetching &&
    isFetchingCartoonDetails
  )
    return <Loader />;
  if (isErrorImages && isError && relatedCartoonsError && isErrorCartoonDetails)
    return <Error />;
    ...
    <>
      <div className="movie__details container">
        <TiArrowBackOutline
          ...
          onClick={() => navigate("/cartoons-page")}
        />
        ...
        <div className="movie__details-preference">
          <h1 className="movie__details-preference__title">
            Preference of subscribing to <span>WORLD.TV</span>
          </h1>
      ...
      <MovieDetailsSwiper images={imagesData} />
      ...
      <CartoonDetailsRelatedList relatedList={relatedCartoons} />
      <Footer />
    </>
  );
};
